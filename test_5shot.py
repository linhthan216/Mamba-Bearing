...
parser.add_argument('--dataset', choices=['CWRU', 'PDB'], help='Choose dataset for training')
parser.add_argument('--training_samples_CWRU', type=int, default=60, help='Number of training samples for CWRU')
parser.add_argument('--training_samples_PDB', type=int, default=30, help='Number of training samples for PDB')
...
parser.add_argument('--bearing_path', default="/content/drive/MyDrive/Bearing_Faults_Mamba/", help="data path")
...
if args.dataset == 'CWRU':
    ...
    seed_func()
    net = CovarianceNet().to(args.device)
    net = torch.load(args.best_weight)
    print('Loading successfully!')
    acc= cal_accuracy_fewshot_5shot(test_dataloader_CWRU, net,args.device, 10)
...
if args.dataset == 'PDB':
    def to_spectrum(data, h=64, w=64, sigma=0.6):
        ...
        return data
    print('Loading data................!!')
    data = torch.load('PU/data.pt')
    label = torch.load('PU/label.pt')
    ...
    net = torch.load(args.best_weight)
    print('Loading successfully!')
    acc= cal_accuracy_fewshot_5shot(test_dataloader_PDB, net,args.device, 10)
    print(f'Accuracy on the test set: {acc:.4f}')
