...
class PatchEmbed2D(nn.Module):
    ...
    """
    def __init__(self, patch_size=3, in_chans=1, embed_dim=96, norm_layer=None, **kwargs):
        super().__init__()
        if isinstance(patch_size, int):
            patch_size = (patch_size, patch_size)
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=3, stride=1, padding='same')
...
class SS_Conv_SSM(nn.Module):
  def __init__(
      ...
      **kwargs,
      ...
      convmix = [ConvMixer(hidden_dim//2, hidden_dim//2,kernel_size=kernel_size) for i in range(depth)]
      self.convmixer = nn.Sequential(*convmix)
  ...
  def forward(self, input: torch.Tensor):
      ...
      input_left = self.convmixer(input_left)
...
class PamMamba(nn.Module):
    def __init__(
        self,
        patch_size=3,
        in_chans=1,
        depths=[2, 3, 2], #số lượng khối SS_Conv_SSM trong mỗi layers
        dims=[16, 32, 64],
        d_state=16,
        drop_rate=0.,
        attn_drop_rate=0.,
        ...
        **kwargs,
        ...
        self.patch_embed = PatchEmbed2D(patch_size=patch_size, in_chans=in_chans, embed_dim=self.dims[0], norm_layer=norm_layer if patch_norm else None)
        ...
        for i_layer in range(self.num_layers):
            layer = VSSLayer(
                ...
                # use_checkpoint=use_checkpoint,  # Ensure use_checkpoint is defined or passed as an argument
            )
